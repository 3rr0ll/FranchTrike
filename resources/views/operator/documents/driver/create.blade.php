    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('operator.documents.driver.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-6">
                    <label for="driver_id" class="block text-sm font-medium text-gray-700 mb-1">Select Driver <span class="text-red-500">*</span></label>
                    <select name="driver_id" id="driver_id" required
                        ...
                        @foreach($drivers as $driverOption)
                            @php
                                // Only show drivers who do NOT have any document in driver_documents table
                                $hasDocuments = \App\Models\DriverDocument::where('driver_id', $driverOption->driver_id)->exists();
                            @endphp
                            @if(!$hasDocuments)
                ...
                <div class="space-y-6">
                    ...
                    <div class="border border-gray-200 rounded-lg p-4">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex-1">
                                ...
                                <div class="mt-2 flex items-center space-x-4">
                                    ...
                                    <a href="{{ $doc->file_url }}" target="_blank" class="text-blue-600 hover:text-blue-800 text-sm">View</a>
                                </div>
                        ...
                        <div class="mt-3">
                            <input type="file" name="documents[{{ $docType->document_id }}]" id="doc_{{ $docType->document_id }}"
                            ...
                            <p class="mt-1 text-xs text-gray-500">Supported formats: PDF, JPG, JPEG, PNG. Max size: 5MB</p>
                        </div>

        ...
        <div class="relative mx-auto my-12 w-full max-w-5xl bg-white rounded-md shadow-lg">
            <div class="flex flex-col p-6" style="min-height: 60vh;">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button onclick="closeResubmitModal()"
                        class="text-gray-400 hover:text-gray-600 transition duration-200">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                ...
                <form id="resubmitForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="dropzone-file"
                            class="flex flex-col items-center justify-center w-full h-96 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-white hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 relative">
                            <div id="file-preview-container"
                                class="absolute inset-0 flex items-center justify-center z-10 hidden bg-white bg-opacity-90 rounded-lg">
                    ...
                    <div class="mt-6 flex justify-end">
                        <button type="button" onclick="closeResubmitModal()"
                            class="mr-2 px-4 py-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 rounded bg-primary-navy text-white hover:bg-primary-gold">Submit</button>
                    </div>
                </form>
            </div>
        </div>
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            var operatorTable = $('#operator-documents-table').DataTable({
                pageLength: 10,
                lengthMenu: [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                ...
                language: {
                    ...
                    zeroRecords: "No operator documents submitted yet.",
                    paginate: {
                        first: "First",
                        last: "Last",
                        next: "Next",
            ...
            // Driver Documents Table
            var driverTable = $('#driver-documents-table').DataTable({
                pageLength: 10,
                lengthMenu: [
                    [10, 25, 50, 100],
                    [10, 25, 50, 100]
                ...
                language: {
                    ...
                    infoEmpty: "Showing 0 to 0 of 0 documents",
                    infoFiltered: "(filtered from _MAX_ total documents)",
                    zeroRecords: "No driver documents submitted yet.",
                    paginate: {
                        first: "First",
                        last: "Last",
                        next: "Next",
            ...
            setTimeout(function() {
                var $opLength = $('#operator-documents-table_wrapper .dataTables_length').css('margin', '0');
                var $opSearch = $('#operator-documents-table_wrapper .dataTables_filter').css('margin', '0');
                if ($opLength.length && $opSearch.length) {
                    $opLength.find('select').addClass(
                        'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg'
                    );
                    $opSearch.find('input').addClass(
                        'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg ml-2'
                    );
                    var $opControls = $('<div class="w-full flex flex-row justify-between items-center mb-4 mr-2"></div>');
                    $opControls.append($opLength).append($opSearch);
                    $opControls.insertBefore($('#operator-documents-table').closest('.overflow-x-auto'));
                }
            }, 0);

            setTimeout(function() {
                var $drLength = $('#driver-documents-table_wrapper .dataTables_length').css('margin', '0');
                var $drSearch = $('#driver-documents-table_wrapper .dataTables_filter').css('margin', '0');
                if ($drLength.length && $drSearch.length) {
                    $drLength.find('select').addClass(
                        'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg'
                    );
                    $drSearch.find('input').addClass(
                        'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg ml-2'
                    );
                    var $drControls = $('<div class="w-full flex flex-row justify-between items-center mb-4 mr-2"></div>');
                    $drControls.append($drLength).append($drSearch);
                    $drControls.insertBefore($('#driver-documents-table').closest('.overflow-x-auto'));
                }
            }, 0);
        });

...
<div class="w-full mx-auto bg-white p-8 rounded-lg shadow">
    ...
    {{-- Show "No changes detected" for any info message containing "No changes detected" --}}
    @if(session('info') && \Illuminate\Support\Str::contains(session('info'), 'No changes detected'))
    <div class="mb-6 flex items-center gap-2 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 px-4 py-3 rounded shadow">
        ...
        <span>
            @if(session('info') === 'No changes detected in profile.')
            No changes detected.
            @elseif(session('info') === 'No changes detected in profile photo.')
            No changes detected.
            @elseif(session('info') === 'New password is the same as the current password.')
            No changes detected.
            @else
            {{ session('info') }}
            @endif
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6 mb-12">
        @csrf
        @method('PUT')
        ...
        <div class="bg-gray-50 rounded-xl shadow p-6 mb-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <img src="{{ $admin->profile_photo_path }}" alt="Profile"
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6 mb-12">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name" class="w-full border border-gray-300 rounded-lg p-2.5" value="{{ old('name', $admin->name) }}">
            @error('name')
        ...
        <div>
            ...
            <input type="email" name="email" id="email" class="w-full border border-gray-300 rounded-lg p-2.5" value="{{ old('email', $admin->email) }}">
            @error('email')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>

...
<body class="font-sans antialiased bg-gray-50">
    ...
    <aside id="logo-sidebar"
        class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-primary-navy border-r border-primary-gold sm:translate-x-0"
        aria-label="Sidebar">
            ...
            <ul class="space-y-2 font-medium mt-4">
                <li>
                    <a href="{{ route('operator.edit') }}"
                        class="flex items-center p-2 gap-2 rounded-lg group text-white hover:bg-primary-gold hover:text-primary-navy">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <li>
                    <form id="logout-form" method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="button"
                            class="flex items-center w-full text-left p-2 rounded-lg text-white hover:bg-accent-red hover:text-white"
                            onclick="confirmLogout(event)">
                            <svg class="w-5 h-5 mr-2 text-white group-hover:text-white" fill="none"
                                stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a2 2 0 1 1-4 0v-1m0-8V5a2 2 0 1 1 4 0v1" />
                            </svg>
                            <span>Sign Out</span>
                        </button>
                    </form>
        ...
        <div class="relative bg-white rounded-sm shadow w-full max-w-lg mx-4">
            ...
            <div class="p-4">
                <div class="flex items-start gap-4 mb-4">
                    <img src="{{ Auth::user()->cloudinary_profile_photo_id ? 'https://res.cloudinary.com/' . config('cloudinary.cloud_name') . '/image/upload/' . Auth::user()->cloudinary_profile_photo_id : (Auth::user()->profile_photo_url ?? asset('images/logo.png')) }}"
                        alt="Profile" class="w-16 h-16 rounded-full object-cover">
                    <div>
                        <p class="text-lg font-semibold text-primary-navy">
                            {{ optional(Auth::user()->operator)->full_name ?? Auth::user()->name }}
                        </p>
                        <p class="text-sm text-gray-600">{{ Auth::user()->email }}</p>
                        <p class="text-sm text-gray-600">
                            {{ Auth::user()->role->name ?? (Auth::user()->getRoleNames()->first() ?? 'Operator') }}
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900">
                            {{ optional(optional(Auth::user()->operator)->birth_date)->format('M d, Y') ?? (optional(Auth::user()->operator)->birth_date ?: '—') }}
                        </p>
                    </div>
                    <div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // var dismissBtn = document.getElementById('dismiss-profile-modal'); // Not used, can be removed
            ...
            // if (dismissBtn) dismissBtn.addEventListener('click', closeModal); // Not used, can be removed
